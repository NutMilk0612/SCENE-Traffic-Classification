...
def assignment(feature_df,threshold):
...
def picture(new_cluster_labels):
    # 绘制重新分配后的聚类结果并保存图像
...
if __name__ == '__main__':
    ...
    true_labels = feature_df.iloc[:, -1].values  # 提取标签列
    #聚类
    cluster_labels = clusting(0.04,4)
    #分配
    new_cluster_labels = assignment(feature_df,1.15)
    #评估
    accuracy, precision, recall, f1 = evaluate_clustering(true_labels, new_cluster_labels)
    print(f"Accuracy: {accuracy}, Weighted Precision: {precision}, Weighted Recall: {recall}, Weighted F1: {f1}")
    #画图
    picture(new_cluster_labels)
